...
void RK4::ComputeStep(double *state_old, double *state_new) {
  // Compute k1
  i_model->EvaluateAt(i_k1, state_old, i_time);
  for (int i = 0; i < i_ndim; ++i) {
    i_temp[i] = state_old[i] + i_h * 0.5 * i_k1[i];
  }

  // Compute k2
  i_model->EvaluateAt(i_k2, i_temp, i_time);
  for (int i = 0; i < i_ndim; ++i) {
    i_temp[i] = state_old[i] + i_h * 0.5 * i_k2[i];
  }

  // Compute k3
  i_model->EvaluateAt(i_k3, i_temp, i_time);
  for (int i = 0; i < i_ndim; ++i) {
    i_temp[i] = state_old[i] + i_h * 0.5 * i_k3[i];
  }
